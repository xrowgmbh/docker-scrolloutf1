...
# test -f /etc/apt/sources.list.d/jitsi-stable.list || echo 'deb https://download.jitsi.org unstable/' >> /etc/apt/sources.list.d/jitsi-stable.list
...
echo '# 0 23 * * * [[ `find /var/lib/amavis/.spamassassin/bayes_{seen,toks} -type f -size +40M > /dev/null 2>&1` ]] && /usr/bin/nice -n 10 su - amavis -c "sa-learn --force-expire" > /dev/null 2>&1' >> /var/spool/cron/crontabs/root
